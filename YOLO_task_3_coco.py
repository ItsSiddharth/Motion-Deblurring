...
BLUR_IMG_DIR = "/home/nam/projects/sid/Motion-Deblurring/coco_val_data_and_annots/blurred_images_coco"
...
OUTPUT_DIR = "task_3_op_coco_post_ft"

os.makedirs(OUTPUT_DIR, exist_ok=True)

# model = YOLO('yolov8m.pt') 
model = YOLO("runs/detect/blur_finetuning/yolov8m_frozen_backbone/weights/best.pt")
...
def collect_mAP_data(bucket, mode='blur'):
    ...
    for img_path, _ in tqdm(bucket, desc=f"mAP {mode}"):
        ...
        test_path = img_path if mode == 'blur' else img_path.replace("blurred_images_coco", "richardson_lucy_deblur_coco")
...
for label, bucket in buckets.items():
    ...
    for img_p, _ in tqdm(bucket, desc="Sample F1"):
        ...
        f1_results['Restored'].append(benchmark_detection(gt, get_yolo_outputs(img_p.replace("blurred_images_coco", "richardson_lucy_deblur_coco"))))
